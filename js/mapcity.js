...
//alert('sp/'+sp+'/'+'ep/'+ep+'/'+'pl/'+pids+'/'+'plse/'+plse+'/')
var c_query='?sp='+sp+'&ep='+ep+'&gotm='+gotm+'&pl='+pids+'&plse='+plse+'&upid='+mapcity.places.upid;
	...
	var _urlconfig = {action:'Get_Plan_map_list',cityid:mapcity.cityid,pids:pids,plse:plse,sp:sp,ep:ep,ruta:ruta,gotm:gotm,upid:mapcity.places.upid,plp:plp,transport:showtransport};
...
//	alert(session.uid);
	...
	this.pageurl.urlquery=$.cookie("EZMAPCITYQUERY")+plp+plt;
...
//		mapcity.getCookieQuery(true);
	...
	$(".map-top-button-hotdeal").click( function(e) {
		e.stopPropagation();e.preventDefault();
		if(mapcity.cityid!="0"){
			var _url = "http://myezplan.com/mobile/appdata/hotdeals.cfm?q=search&city="+mapcity.cityid+"&#SearchResults";
	...
	$('#go_mapcitypopup').click( function(e) {
		...
		if(mapcity.cityid!="0"){
			var _url = "mapcitypopup.html?ctid="+mapcity.cityid+"&plid="+mapcity.countryid;
			window.top.location.href=_url;
		}
		return true;
	});
